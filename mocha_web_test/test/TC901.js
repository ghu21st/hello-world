...
//TC901: Test send input model name with Protractor on AngularJS sample app
//QA test setup ----------------------------------------------
var case_ID = 901;
...
//-------------------------------------------------------
describe('TC901: Test send input model name with Protractor on AngularJS sample app', function() {
...
//-------------------------------------------------------
    it('TC901: Test send input model name with Protractor on AngularJS sample app', function(done) {
        //trace HTTP call
        nock.recorder.rec();

        test_url = 'http://www.angularjs.org';  //change the default tested url set from config file
        ...
        async.series([
            ...
            function(callback){
                element1 = ptor.element(protractor.By.model('yourName'));
                element1.sendKeys('This is a QA test').then(function(){
                    output_msg = '\nSet model youName ';
                    output_string += output_msg;
                    console.log(output_msg);
                    //
            ...
            function(callback){
                element2 = ptor.element(protractor.By.binding('yourName'));
                element2.getText().then(function(txt){
                    output_msg = '\nGet binding yourName return text:' + txt;
                    output_string += output_msg;
                    console.log(output_msg);
                        //
                    var result = [txt, 0];
            ...
            function(err, result){
                ...
                try{
                    ...
                    expect(result[2][0]).to.match(/AngularJS/);
                    expect(result[4][0]).to.match(/Hello This is a QA test!/);
